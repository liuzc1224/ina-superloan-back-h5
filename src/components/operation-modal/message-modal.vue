      ...
      <Modal v-model="showModal" :title="formValidate.id? $t('messagePush.editMessage') : $t('messagePush.addMessage')" :mask-closable="false">
        ...
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">
          ...
          <FormItem :label="$t('messagePush.sendChannel')" prop="sendChannel">
            <Select v-model="formValidate.sendChannel" :placeholder="$t('messagePush.selectChannel')">
          ...
          <FormItem :label="$t('messagePush.messageType')" prop="msgType">
            <Select v-model="formValidate.msgType" :placeholder="$t('messagePush.selectType')">
          ...
          <FormItem :label="$t('messagePush.sendTarget')" prop="sendTarget">
            <Select v-model="formValidate.sendTarget" :placeholder="$t('messagePush.selectTarget')">
              <Option :value="0">{{$t('messagePush.allPeople')}}</Option>
            </Select>
          </FormItem>
          <FormItem :label="$t('messagePush.messageText')" prop="text">
            <Input v-model="formValidate.text" type="textarea" :autosize="{minRows: 2,maxRows: 5}" :placeholder="$t('messagePush.inputText')"></Input>
          </FormItem>
        </Form>
        <div slot="footer">
